...
import { Button, Col, Container, Row, Stack, Form, Card, Badge, Modal, ModalBody } from "react-bootstrap";
...
export function NoteList({ availableTags, notes , deleteTag , updateTag  }: NoteListProps) {
    ...
    return <>
        ...
        <EditTagsModal deleteTag={deleteTag}  updateTag={updateTag} show={editTagsModalIsOpen} availableTags={availableTags} handleClose={() =>  setEditTagsModalIsOpen(false) }/>
    </>
}
                    ...
                    <Form>
                        <Stack gap={2}>
                            {availableTags.map(tag => (
                                <Row key={tag.id}>
                                    <Col>
                                        <Form.Control onChange={(e) => updateTag(tag.id , e.target.value)} value={tag.label} type="text"/>
                                    </Col>
                                    <Col xs="auto">
